...
REPORTS_DIR = Path(__file__).parent / "seo_reports"
...
def audit_meta_descriptions(pages, posts):
    log("🔍", "Starting meta description audit...")
    changes = []
    skipped = 0
    pages_fixed = 0
    posts_fixed = 0
    ...
    def process_item(item, obj_type):
        ...
        if new_desc:
            saved = False
            # Try Rank Math API first; fall back to WP REST meta update
    ...
    log("📊", f"Meta desc: {pages_fixed} pages fixed, {posts_fixed} posts fixed, {skipped} skipped")
...
def cleanup_duplicate_posts(posts):
    log("🔍", "Scanning for duplicate slugs...")
    redirects_created = 0
    posts_trashed = 0
    changes = []
    ...
    for item in posts:
        ...
        try:
            ...
            log("⚠️", f"Slug lookup failed for '{canonical_slug}': {exc}")
            continue
        ...
        if canonical_results:
            ...
            try:
                ...
                time.sleep(RATE_LIMIT_SLEEP)
                log("↪️", f"301 redirect: {url} → {canonical_url}")
                redirects_created += 1
                changes.append({
                ...
                log("⚠️", f"Redirect creation failed for post {obj_id}: {exc}")
        ...
        try:
            ...
            log("⚠️", f"Trash failed for post {obj_id}: {exc}")

    log("📊", f"Duplicates: {redirects_created} redirects created, {posts_trashed} posts trashed")
...
def fix_duplicate_meta_tags(pages):
    log("🔍", "Checking for duplicate <meta name=description> tags...")
    fixed = 0
    changes = []
    ...
    for item in pages:
        ...
        log("⚠️", f"[page {obj_id}] {len(live_descs)} meta descriptions found: {url}")

        try:
            wp_post(f"pages/{obj_id}", {"excerpt": ""})
            time.sleep(RATE_LIMIT_SLEEP)
        except Exception as exc:
            log("⚠️", f"[page {obj_id}] excerpt clear failed: {exc}")
        ...
        try:
            ...
            time.sleep(RATE_LIMIT_SLEEP)
            log("✅", f"[page {obj_id}] deduped meta description")
            fixed += 1
            changes.append({
            ...
            log("⚠️", f"[page {obj_id}] updateMeta failed: {exc}")

    log("📊", f"Duplicate meta tags: {fixed} pages fixed")
...
def check_sitemap_health():
    log("🔍", "Checking sitemap for redirects...")
    redirected_urls = []
    ...
    if not sitemap_content:
        log("⚠️", "No sitemap found at expected locations")
    ...
    log("ℹ️", f"Checking {len(page_urls)} URLs from sitemap...")
    ...
    for url in page_urls:
        try:
            ...
            if 300 <= resp.status_code < 400:
                ...
                log("↪️", f"[{resp.status_code}] {url} → {dest}")
            time.sleep(RATE_LIMIT_SLEEP)
        except Exception as exc:
            log("⚠️", f"HEAD request failed for {url}: {exc}")

    log("📊", f"Sitemap: {len(page_urls)} URLs checked, {len(redirected_urls)} have redirects")
    return {
...
def write_report(meta_results, dupe_results, dup_meta_results, sitemap_results):
    ...
    lines = [
        ...
        f"- Skipped (no URL / not indexable / already OK): **{meta_results['skipped']}**",
        "",
    ]
    ...
    if sitemap_results["redirected_urls"]:
        lines.append("### ⚠️ Manual Attention Required — Redirected Sitemap URLs")
        lines.append("")
        lines.append("These URLs appear in the sitemap but return a redirect. Update the sitemap "
                     "or fix the canonical URL:")
        lines.append("")
